...
namespace AssetCatalog.Editor
{
    ...
    public class CatalogGeneratorEditor : UnityEditor.Editor
    {
        ...
        private SerializedProperty _syncScrollGloballyProperty;
        ...
        private void OnEnable()
        {
            ...
            _syncScrollGloballyProperty = serializedObject.FindProperty("syncScrollGlobally");
        ...
        public override void OnInspectorGUI()
        {
            ...
            EditorGUILayout.PropertyField(_syncScrollGloballyProperty, new GUIContent("Sync Scroll Globally"));
        ...
        private void CreateScrollView()
        {
            ...
            ConfigureGlobalScrollSync(scrollView, scrollRect, _target.syncScrollGlobally);

            // Ensure all generated UI objects under CanvasRoot stay on the Default layer.
            SetLayerRecursively(canvasRoot, defaultLayer);
        }

        private static void ConfigureGlobalScrollSync(GameObject scrollView, ScrollRect scrollRect, bool enableSync)
        {
            if (scrollView == null) return;

            var syncType = FindType("AssetCatalog.ScrollRectSyncUdon");
            if (syncType == null) return;

            var existing = scrollView.GetComponent(syncType);
            if (!enableSync)
            {
                if (existing != null)
                {
            ...
            var syncComponent = existing ?? scrollView.AddComponent(syncType);
            if (syncComponent == null) return;

            var scrollRectField = syncType.GetField("targetScrollRect");
            if (scrollRectField != null && scrollRectField.FieldType == typeof(ScrollRect))
            {
                scrollRectField.SetValue(syncComponent, scrollRect);
            }
        }
